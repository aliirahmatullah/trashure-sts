...
<div class="container mx-auto mt-4">
    ...
    <div class="mb-4 flex items-center justify-between">
        <h1 class="text-xl font-semibold text-gray-800">Data Transaksi (Trash)</h1>
    ...
    <div class="overflow-hidden rounded-lg border border-gray-200 shadow-sm">
        <table class="min-w-full divide-y divide-gray-200 text-sm text-gray-700">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                @forelse ($transaction as $key => $trash)
                <tr class="transition hover:bg-gray-50">
                    ...
                    <td class="whitespace-nowrap px-6 py-3 text-center">
                        <span class="rounded-full px-3 py-1 text-xs font-semibold
                            @if($trash->status === 'pending')   bg-yellow-200 text-yellow-700
                            @elseif($trash->status === 'approved') bg-blue-100 text-blue-700
                            @elseif($trash->status === 'completed') bg-green-100 text-green-700
                            @elseif($trash->status === 'canceled') bg-red-100 text-red-700
                            @endif">
                            {{ ucfirst($trash->status) }}
                        </span>
                    </td>
                    <td class="whitespace-nowrap px-6 py-3 text-center">
                        <div class="flex items-center justify-center gap-2">
                            {{-- Kembalikan --}}
                            <form action="{{ route('admin.transactions.restore', $trash->id_transaksi) }}" method="POST">
                                @csrf @method('PATCH')
                                <button type="submit"
                            ...
                            {{-- Hapus Permanen --}}
                            <form action="{{ route('admin.transactions.delete_permanent', $trash->id_transaksi) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
