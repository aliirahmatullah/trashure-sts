...
<div class="min-h-screen bg-gray-50 py-10">
    <form action="{{ route('admin.waste-types.update', $wasteType['id_jenis']) }}" method="POST"
        ...
        <div class="mb-5">
            ...
            <div class="relative">
                ...
                <input type="text" name="nama_jenis" placeholder="Masukkan jenis sampah"
                       ...
                       value="{{ old('nama_jenis', $wasteType['nama_jenis']) ?? '' }}">
            </div>
        ...
        <div class="mb-5">
            ...
            <div class="relative">
                ...
                <input type="number" name="poin_per_kg" placeholder="Masukkan jumlah point per kg"
                       ...
                       value="{{ old('poin_per_kg', $wasteType['poin_per_kg']) ?? '' }}">
            </div>
        ...
        <div class="mb-5">
            ...
            <div class="relative">
                ...
                <select name="kategori"
                    ...
                    <option value ="" disabled {{ !old('kategori', $wasteType['kategori'] ?? '') ? 'selected' : ''}} >Pilih kategori</option>
                    <option value="Plastik" {{ old('kategori', $wasteType['kategori'] ?? '') == 'Plastik' ? 'selected' : '' }}>Plastik</option>
                    <option value="Kertas" {{ old('kategori', $wasteType['kategori'] ?? '') == 'Kertas' ? 'selected' : '' }}>Kertas</option>
                    <option value="Logam" {{ old('kategori', $wasteType['kategori'] ?? '') == 'Logam' ? 'selected' : '' }}>Logam</option>
                    <option value="Kaca" {{ old('kategori', $wasteType['kategori'] ?? '') == 'Kaca' ? 'selected' : '' }}>Kaca</option>
                </select>
            </div>
            ...
            <div class="relative">
                ...
                <textarea name="deskripsi_jenis" rows="4" placeholder="Tulis deskripsi singkat"
                          class="w-full resize-none rounded-lg border border-gray-300 pl-10 pr-4 py-2.5 text-sm
                                 focus:border-green-500 focus:ring-green-400 focus:outline-none focus:ring-1
                                 @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi_jenis', $wasteType['deskripsi_jenis'] ?? '') }}</textarea>
            </div>
