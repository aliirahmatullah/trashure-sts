...
<div class="min-h-screen bg-gray-50 py-10">
    <form action="{{ route('staff.transactions.store') }}" method="POST"
        ...
        <h2 class="mb-6 text-lg font-semibold text-gray-800">Tambah Transaksi</h2>
        ...
        {{-- Lokasi (readonly, 1 pilihan) --}}
        <div class="mb-5">
            <label class="mb-1 flex items-center text-sm font-medium text-gray-700">
                Lokasi Penukaran
            </label>
            <select name="id_lokasi" disabled
                    class="w-full rounded-lg border border-gray-300 bg-gray-100 px-3 py-2.5 text-sm
                           focus:border-green-500 focus:ring-green-400 focus:outline-none focus:ring-1">
                <option value="{{ auth()->user()->id_lokasi }}" selected>
                    {{ auth()->user()->location->nama_lok }} - {{ auth()->user()->location->kota }}
                </option>
            </select>
            {{-- tetap kirim value --}}
